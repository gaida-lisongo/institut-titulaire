...
import { Student, Unite, Element } from '@/types/jury';
...
export default function StudentModal({ student, unites, onClose }: StudentModalProps) {
  ...
  const fetchNote = async ({
    ...
    try {
      ...
      if (request.success) {
        ...
        return request.data || []; // Assurer qu'on retourne au moins un tableau vide
      }
  ...
  useEffect(() => {
    ...
    // Vérifier si l'étudiant a un id_etudiant avant de charger les notes
    ...
    // Créer un tableau de promesses pour toutes les requêtes de notes
    const fetchPromises: Promise<void>[] = [];

    unites.forEach(unite => {
      // Vérifier si l'unité a des éléments et un id_annee avant de charger les notes
      ...
      unite.elements.forEach(element => {
        const promise = fetchNote({
          ...
          .then(noteData => {
            // Vérifier si on a des données de note
            if (noteData && noteData.length > 0) {
              ...
              // Pas de note trouvée, initialiser à 0
    ...
    // Attendre que toutes les requêtes de notes soient terminées
  ...
  const handleSaveNotes = async () => {
    setIsSaving(true);
    try {
      // Parcourir toutes les notes modifiées et les envoyer au serveur
      const savePromises = Object.entries(notes).map(async ([elementIdStr, noteValue]) => {
        ...
        // Si on a des infos sur cette note, c'est une mise à jour
        if (noteInfo) {
          // Ici, il faudrait implémenter la logique de mise à jour de la note
        ...
        <div className="flex-1 overflow-auto">
          {loading ? (
            ...
            <div className="flex flex-col md:flex-row">
              ...
              {/* Accordéon des UE et éléments - 2/3 */}
