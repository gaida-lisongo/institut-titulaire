...
export default function TravauxDetail() {
  ...
  const fetchTravaux = async () => {
    try {
      const request = await titulaireApi.fetchTravauxByCharge(id);
      console.log("Travaux fetched:", request);
      if (request.success) {
        setTravaux(request.data);
      } else {
        setError(request.message || "Erreur lors de la récupération des travaux");
      }
    } catch (error) {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {/* Ajouter le composant Travaux */}
      <Travaux 
        chargeId={Number(id)} 
        travaux={travaux} 
        onUpdate={fetchTravaux} 
      />
    </div>
  );
}