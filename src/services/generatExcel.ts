...
class FicheCotation {
   ...
   */
  public async generateAndDownloadExcel(
    students: Student[],
    ...
    try {
      ...
      students.forEach((student) => {
        const { totalObtenu, mention } = this.calculateTotals(
          String(student.cmi ?? ""),
          student.examen != null ? String(student.examen) : null,
          student.rattrapage != null ? String(student.rattrapage) : null,
          courseInfo.credit,
        );
      ...
      const passCount = students.filter((s) => {
        const { totalObtenu } = this.calculateTotals(
          String(s.cmi ?? ""),
          s.examen != null ? String(s.examen) : null,
          s.rattrapage != null ? String(s.rattrapage) : null,
          courseInfo.credit,
        );
      ...
      const statsRows = [
        ...
        [
          "Réussite",
          `${passCount} (${Math.round((passCount / totalStudents) * 100)}%)`,
        ],
        [
          "Échec",
          `${failCount} (${Math.round((failCount / totalStudents) * 100)}%)`,
        ],
      ];
      ...
      worksheet.mergeCells(
        `A${statsHeaderRow.number}:J${statsHeaderRow.number}`,
      );
   ...
   */
  public async generateExcelBuffer(
    students: Student[],
    courseInfo: CourseInfo,
  ): Promise<ArrayBuffer> {
    // Créer un nouveau classeur
